...
const ClinicDashboard: React.FC = () => {
  ...
  // ÉTAT POUR LE CONTRÔLE DE L'IMPRESSION MOBILE (NOUVEAU)
  const [isPrinting, setIsPrinting] = useState<'accueil' | 'caisse' | null>(null);
  ...
  // NOUVELLE FONCTION D'IMPRESSION MOBILE (SANS POP-UP)
  const imprimerTicketAccueil = () => {
    if (!ticketGenere) return;
    setIsPrinting('accueil');
    setTimeout(() => {
      window.print();
      setIsPrinting(null);
    }, 800); // Laisse le temps au QR code de s'afficher
  };
  ...
  const lancerImpressionThermique = (transaction: RapportVente) => {
    setIsPrinting('caisse');
    setTimeout(() => {
      window.print();
      setIsPrinting(null);
      setRecuApercu(null); // Ferme l'aperçu après impression
    }, 800);
  };
  ...
  // Impression des codes barre (L'administrateur est sur Ordinateur, donc window.open() marche très bien ici)
  ...
  return (
    <>
      {/* ========================================================================================= */}
      {/* MODE IMPRESSION MOBILE (VISIBLE UNIQUEMENT LORSQUE L'ON CLIQUE SUR IMPRIMER)              */}
      {/* ========================================================================================= */}
      <style>{`
        @media print {
      ...
      <div className="hidden print:block bg-white text-black w-[58mm] absolute top-0 left-0 m-0 p-2 z-[9999]">
        
        {/* TICKET ACCUEIL NOUVEAU STYLE (MOBILE) */}
        {isPrinting === 'accueil' && ticketGenere && (
          <div className="text-center font-sans">
            <div className="font-bold text-[16px]">Clinique Ong Notre Grenier</div>
            <div className="text-[12px] mb-2">{new Date().toLocaleDateString()} - {ticketGenere.heureArrivee}</div>
            <div className="text-[14px]">Ticket d'attente</div>
            <div className="text-[28px] font-bold border-2 border-black p-1 my-2 mx-2">{ticketGenere.ticket}</div>
            <div className="text-[14px] font-bold">{ticketGenere.nom}</div>
            <div className="text-[14px] font-bold mt-2 border-t border-dashed border-black pt-1">Frais : {PRIX_CONSULTATION[ticketGenere.service]} FCFA</div>
            <img className="w-[120px] h-[120px] mx-auto my-2 block" src={`https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${ticketGenere.dossierId}`} alt="QR" />
            <div className="text-[10px]">Conservez ce ticket pour la caisse</div>
          </div>
        )}

        {/* TICKET CAISSE NOUVEAU STYLE (MOBILE) */}
        {isPrinting === 'caisse' && recuApercu && (
          <div className="text-center font-sans">
            <div className="font-bold text-[16px] mb-1">Clinique ONG Grenier</div>
            <div className="text-[12px]">Reçu de Pharmacie</div>
            <div className="text-[10px] mb-2">Le {recuApercu.date} à {recuApercu.heure}</div>
            <div className="border-t border-dashed border-black my-1"></div>
            <div className="text-left text-[12px]">Ticket N°: <span className="font-bold">{recuApercu.id}</span></div>
            <div className="text-left text-[12px]">Patient: <span className="font-bold">{recuApercu.patientNom}</span></div>
            <div className="text-left text-[12px]">Caissier: {loggedInUser?.nomComplet}</div>
            <div className="border-t border-dashed border-black my-1"></div>
            <div className="flex justify-between font-bold text-[12px]"><span>Désignation</span><span>Prix</span></div>
            <div className="border-t border-dashed border-black my-1"></div>
            {recuApercu.detailsPanier.map((l, idx) => (
              <div key={idx} className="text-left text-[11px] mb-1 flex justify-between items-start">
                <span className="flex-1 pr-1 leading-tight">{l.quantite}x {l.medicament.nom}</span>
                <span className="font-bold">{l.quantite * l.medicament.prix}F</span>
              </div>
            ))}
            <div className="border-t border-dashed border-black my-1"></div>
            <div className="flex justify-between font-bold text-[16px]"><span>TOTAL:</span><span>{recuApercu.montant} F</span></div>
            <div className="border-t border-dashed border-black my-1"></div>
            <img className="w-[120px] h-[120px] mx-auto my-2 block" src={`https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${recuApercu.id}`} alt="QR Code Reçu" />
            <div className="text-[10px]">Prompt rétablissement !</div>
          </div>
        )}
      </div>

      {/* ========================================================================================= */}
      {/* APPLICATION PRINCIPALE (TOTALEMENT CACHÉE PENDANT L'IMPRESSION GRÂCE AU `print:hidden`)   */}
      {/* ========================================================================================= */}
      <div className="h-[100dvh] flex flex-col bg-slate-50 font-sans overflow-x-hidden w-full print:hidden">
        {/* HEADER MÉDICAL */}
                    ...
                    {recuApercu && (
                      <div className="absolute inset-0 bg-blue-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                        <div className="bg-slate-50 p-6 rounded-3xl shadow-2xl flex flex-col items-center max-w-sm w-full border border-blue-200">
                          ...
                          <div className="bg-white p-4 w-[58mm] min-h-[100mm] shadow-md mb-6 font-mono text-[10px] text-black border border-slate-300 mx-auto rounded-lg">
